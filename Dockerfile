...
RUN --mount=type=cache,id=cacheKey,target=/var/cache/apt,sharing=shared \
  --mount=type=cache,id=cacheKey,target=/var/lib/apt/lists,sharing=shared \
  apt-get update \
  && apt-get upgrade -y \
...
RUN --mount=type=cache,id=cacheKey,target=/root/.npm \
...
COPY . ./
RUN --mount=type=cache,id=cacheKey,target=/app/.build/next/cache \
...
RUN --mount=type=cache,id=cacheKey,target=/var/cache/apt,sharing=locked \
  --mount=type=cache,id=cacheKey,target=/var/lib/apt/lists,sharing=locked \
  apt-get update \
